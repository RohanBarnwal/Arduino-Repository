from scipy.spatial import distance
from imutils import face_utils
import imutils
import dlib
import cv2
import telebot
import threading
import serial
import pygame

# Initialize the Telebot with your API token
bot = telebot.TeleBot("TELEBOT API")

# Define your chat ID
chat_id = " "

def eye_aspect_ratio(eye):
    A = distance.euclidean(eye[1], eye[5])
    B = distance.euclidean(eye[2], eye[4])
    C = distance.euclidean(eye[0], eye[3])
    ear = (A + B) / (2.0 * C)
    return ear

thresh = 0.25
frame_check = 20
detect = dlib.get_frontal_face_detector()
predict = dlib.shape_predictor("models/shape_predictor_68_face_landmarks.dat")

(lStart, lEnd) = face_utils.FACIAL_LANDMARKS_68_IDXS["left_eye"]
(rStart, rEnd) = face_utils.FACIAL_LANDMARKS_68_IDXS["right_eye"]
cap = cv2.VideoCapture(0)
flag = 0
drowsy_detected = False

# Initialize the serial connection to the Arduino
ser = serial.Serial('COM3', 9600)  # Adjust the port and baud rate accordingly

# Initialize Pygame for audio playback
pygame.mixer.init()
alert_sound = pygame.mixer.Sound("alert.wav")

def handle_message(message):
    global drowsy_detected
    
    if message.text.lower() == "enable it":
        bot.send_message(chat_id, "OK, ACTIVATED!")
        ser.write(b'D')  # Send 'D' to Arduino
        drowsy_detected = True
    elif message.text.lower() == "disable it":
        bot.send_message(chat_id, "Alright, resuming drowsiness detection.")
        ser.write(b'B')  # Send 'B' to Arduino
        drowsy_detected = False

@bot.message_handler(func=lambda message: True)
def reply_hello(message):
    threading.Thread(target=handle_message, args=(message,)).start()

def drowsiness_detection():
    global drowsy_detected
    
    while True:
        ret, frame = cap.read()
        
        if not ret:
            break
        
        frame = imutils.resize(frame, width=450)
        gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
        subjects = detect(gray, 0)
        
        for subject in subjects:
            shape = predict(gray, subject)
            shape = face_utils.shape_to_np(shape)
            leftEye = shape[lStart:lEnd]
            rightEye = shape[rStart:rEnd]
            leftEAR = eye_aspect_ratio(leftEye)
            rightEAR = eye_aspect_ratio(rightEye)
            ear = (leftEAR + rightEAR) / 2.0
            leftEyeHull = cv2.convexHull(leftEye)
            rightEyeHull = cv2.convexHull(rightEye)
            cv2.drawContours(frame, [leftEyeHull], -1, (0, 255, 0), 1)
            cv2.drawContours(frame, [rightEyeHull], -1, (0, 255, 0), 1)
            
            if ear < thresh and not drowsy_detected:
                flag += 1
                print(flag)
                
                if flag >= frame_check:
                    cv2.putText(frame, "Drowsy", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
                    cv2.putText(frame, "Drowsy", (10, 325), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
                    
                    if not drowsy_detected:
                        bot.send_message(chat_id, "DROWSINESS DETECTED DO YOU WANT TO ENABLE SAFETY MODE")
                        alert_sound.play()  # Play the alert sound
                        drowsy_detected = True
            else:
                flag = 0
        
        cv2.imshow("Frame", frame)
        key = cv2.waitKey(1) & 0xFF
        
        if key == ord("q"):
            break

    cv2.destroyAllWindows()
    cap.release()

# Start the drowsiness detection thread
detection_thread = threading.Thread(target=drowsiness_detection)
detection_thread.start()

# Start the bot's polling loop
bot.polling()
